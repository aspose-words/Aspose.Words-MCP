...
def test_table_formatting_suite():
    ...
    did = r['docId']
    t = srv.tool_add_table_end(did, 3, 3, data=[['H1', 'H2', 'H3'], ['A', 'B', 'C'], ['D', 'E', 'F']], has_header_row=True)
    tidx = t['tableIndex']
    srv.tool_format_table(did, tidx, has_header_row=True, border_style='single', shading=[[1, 1, 'FFEEEE']])
...
def test_add_table_start_and_at_paragraph_and_merge_cells():
    ...
    t2 = srv.tool_add_table_at_paragraph(did, 2, 3, paragraph_index=idx_para, data=[["x","y","z"],["u","v","w"]], has_header_row=False)
    assert isinstance(t2['tableIndex'], int)
    srv.tool_merge_table_cells(did, table_index=t2['tableIndex'], start_row=0, start_col=0, end_row=0, end_col=1)
