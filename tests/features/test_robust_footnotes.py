...
def test_footnotes_by_anchor_add_validate_delete():
    ...
    srv.tool_add_footnote_by_anchor(
        did, anchor_text='ANCHOR', text='FN1', position='after', occurrence=1
    )
    v = srv.tool_validate_footnotes_by_anchor(did, anchor_text='ANCHOR', min_count=1)
    assert v['ok'] is True
    assert v['count'] >= 1
    srv.tool_add_footnote_by_anchor(
        did, anchor_text='ANCHOR', text='FN2', position='before', occurrence=2
    )
    ...
    del_one = srv.tool_delete_footnotes_by_anchor(
        did, anchor_text='ANCHOR', occurrence=1, remove_all=False
    )
