...
def header_add_text(doc_id: str, text: str, primary: bool = True) -> bool:
    ...
    builder.move_to_header_footer(
        aw.HeaderFooterType.HEADER_PRIMARY if primary else aw.HeaderFooterType.HEADER_FIRST
    )
...
def footer_add_text(doc_id: str, text: str, primary: bool = True) -> bool:
    ...
    builder = aw.DocumentBuilder(doc)
    builder.move_to_header_footer(
        aw.HeaderFooterType.FOOTER_PRIMARY if primary else aw.HeaderFooterType.FOOTER_FIRST
    )
    builder.writeln(text)
    doc.save(str(path))
    return True
...
def insert_section_break(doc_id: str, kind: str = 'nextPage') -> bool:
    ...
    builder = aw.DocumentBuilder(doc)
    mapping = {
        'nextpage': aw.BreakType.SECTION_BREAK_NEW_PAGE,
        'continuous': aw.BreakType.SECTION_BREAK_CONTINUOUS,
        'even': aw.BreakType.SECTION_BREAK_EVEN_PAGE,
        'odd': aw.BreakType.SECTION_BREAK_ODD_PAGE,
    }
